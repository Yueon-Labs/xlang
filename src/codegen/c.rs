...
impl CGen {
    ...
    fn emit_runtime_preamble(&mut self) {
        let lines = [
            ...
            "    return out;",
            "}",
            "// int_to_str returns a pointer into a rotating pool of 8 buffers",
            "// (the glibc-asprintf/itoa trick). Avoids a malloc+free PER call —",
            "// the old version did malloc(16) on every int_to_str, which dominated",
            "// number-formatting loops (10–18x slower than GNU coreutils). Safe for",
            "// up to 8 live results in one expression (rare to exceed).",
            "static char __xlang_itoa_pool[8][16];",
            "static int __xlang_itoa_idx = 0;",
            "char* __xlang_int_to_str(int32_t n) {",
            "    char* buf = __xlang_itoa_pool[__xlang_itoa_idx];",
            "    __xlang_itoa_idx = (__xlang_itoa_idx + 1) & 7;",
            "    snprintf(buf, 16, \"%d\", n);",
            "    return buf;",
            "}",
